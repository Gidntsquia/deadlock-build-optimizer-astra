...
| Default Infernus; two named builds; ≥12 items, phases, costs, totals, images  | **Updated at user request:** one consensus build with 15 ordered purchases in three phases. All 15 image/detail bindings tested.                                                                                             |
...
| Item detail cards match asset data                                            | **Pass.** Image, catalog cost, tier, slot, every displayed stat value/label, and every description tested for all 15 recommended Infernus cards. Missing-value asset properties are omitted.                                    |
...
| 390×844 layout and ≥40px tap targets                                          | **Pass.** All hero/build/view combinations and 15 detail dialogs tested for horizontal overflow and target dimensions. Escape closes the dialog and restores focus. Desktop stays in a centered 680px column.                   |
...
Evidence is in `verification/`: `offline-verification.log`, `browser-report.json`, `build-hashes.json`, `generated-infernus.json`, `validation-report.json`, mobile screenshots for items/abilities/validation/detail, and a desktop screenshot. The algorithm suite has **11 passing tests and 1 explicitly skipped upstream constraint**, with no failures.
...
| Game phase             |    10% | `exp(-abs(avg_buy_time_s - phaseTarget) / 600)`, targets 240/900/1680 seconds. Unknown timing scores 0.2.                                                                                      |
...
The consensus build selects 15 unique purchases: five early (tier 1), five mid (tiers 2–3), five late (tiers 3–4). Category quotas are removed. Focus comes from total offensive-category usage. At each step, rank legal candidates, shortlist the strongest remaining phase purchases, then schedule the earliest observed purchase with components before their upgrades.
...
**Overall agreement = round(100 × (0.70 × overlap + 0.30 × order)).** This measures how well the generator did against one player's core; it is not build strength, a confidence interval, or predicted win probability. The consensus revision yields **45%**, up from the previous best **35%**: 9/24 core items, 29.99% weighted overlap, and 80.56% agreement across 36 shared-item pairs. The evaluator, source snapshots, and sample are unchanged. Infernus uses the all-skill fallback. This is a reused development benchmark, not fresh independent generalization evidence; it is small, older, and drawn from one player.
